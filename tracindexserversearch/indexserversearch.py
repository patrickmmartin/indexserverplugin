...
class IndexServerProxy(Component):
    ...
    def get_index_server_results(self, repo, wordlist):
        ...
        checkoutroot = self.env.config.get(
            'indexserver-search', 'catalogue-root', '')
        CoInitialize()
        try:
            ...
            while not (result.EOF):
                ...
                # index might not be up completely up to date, and has old
                # files
                if fso.FileExists(indexpath):
                    ...
                    try:
                        ...
                        self.env.log.warning(
                            'ignoring failure to obtain node at ' + url + ': ' + e)
                    ...
                    if node:
                        ...
                        if ((node.get_content_type() == "application/octet-stream") and (characterization <> None)):
                            try:
                                content = to_unicode(
                                    characterization, "utf_16")
                                self.env.log.debug(
                                    'got characterization field for node ' + url)
                            except UnicodeEncodeError, e:
                                content = None
                                self.env.log.warning(e)
                                self.env.log.warning(
                                    'ignoring failure for conversion of characterization field for node ' + url)
                        ...
                        if not content:
                            ...
                            if contentstream:
                                content = to_unicode(
                                    contentstream.read(), node.get_content_type())
                                self.env.log.debug(
                                    'got content from repository for node ' + url)
                            else:
                                self.env.log.debug(
                                    'no content from repository for node ' + url)
